...
const createDivsForChars = (word) => {
  ...
  'beforeend', `<div class = "letter-box" ${letter}"></div>`);
  }
};
...
const generateLetterButtons = () => {
  ...
  for (const letter of ALPHABET){
    button.insertAdjacentHTML(
      'beforeend', `<button> ${letter}</button>`);
...
const isLetterInWord = (letter) => {
  const check = document.querySelector(`div.${letter}`)
  if( check !== null){
    return true
  }else{
    return flase
